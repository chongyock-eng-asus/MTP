...
if __name__ == "__main__":
    ...
    # Initialize tokenizer and extend vocabulary
    tokenizer = AutoTokenizer.from_pretrained("TinyLlama/TinyLlama-1.1B-Chat-v1.0")
    if tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token

    mask_token_ids = model.extend_vocabulary(tokenizer)
    print(f"Added mask tokens with IDs: {mask_token_ids}")
    ...
    ntp_mask = torch.ones(1, seq_len, dtype=torch.bool)
    mtp_mask = torch.zeros(1, seq_len, dtype=torch.bool)
    ...
    print(f"Sampler logits shape: {outputs['sampler_logits'].shape}")
