...
def create_simple_dataloader(sequences: List[List[int]], tokenizer, batch_size: int = 4):
    ...
    class SimpleDataset(Dataset):
        ...
        def __getitem__(self, idx):
            ...
            labels = sequence[1:] + [self.tokenizer.eos_token_id]

            # Pad sequences
            input_ids = sequence + [self.tokenizer.pad_token_id] * (self.max_length - len(sequence))
            labels = labels + [-100] * (self.max_length - len(labels))
            attention_mask = [1] * len(sequence) + [0] * (self.max_length - len(sequence))

            # Simple NTP/MTP masks (all NTP for basic training)
            ntp_mask = [True] * len(sequence) + [False] * (self.max_length - len(sequence))
            mtp_mask = [False] * self.max_length
    ...
    dataset = SimpleDataset(sequences, tokenizer)
